...
function bindOsc(oscServer, oscClient, io) {
    ...
    oscServer.on(node_path("/alert"), function(msg, rinfo) {
        postln('alert message: *' + msg[1] + '* - ' + msg[2]);
        io.emit(cli_path("/modal/alert"), {'title': msg[1], 'text': msg[2]});
    });

    oscServer.on(node_path("/supercollider"), function(msg, rinfo) {
        postln('message from SuperCollider server: ' + msg[1] + ' ' + msg[2]);
        io.emit(cli_path("/supercollider"), msg[1], msg[2]);
    });
}
