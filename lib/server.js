...
function syncClient(url) {
    if (sync_dict[url] !== undefined) {
        var osc_path = sc_path("/app/sync") + url;
        osc_client.send(osc_path);
    }
}
...
function registerHttpCallbacks(app) {
    ...
    // serve SFW files
    app.get('/js/lib/*.swf', function(req, res) {
        getHttp(res, req['path']);
    });
}
...
function bindOsc(oscServer, oscClient, io) {
    ...
    oscServer.on(node_path("/app/sync/add"), function(msg, rinfo) {
        sync_dict[msg[1]] = true;
    });

    oscServer.on(node_path("/app/sync/remove"), function(msg, rinfo) {
        delete sync_dict[msg[1]];
    });

    oscServer.on(node_path("/app/sync/list"), function(msg, rinfo) {
        postmsg(JSON.stringify(sync_dict));
    });
}
...
function bindSocket(io, socket, oscClient) {
    socket.on(node_path("/supercollider"), function(msg) {
        ...
        if (msg.length == 1) {
            oscClient.send(sc_path("/control"), msg[0]);
        }
        if (msg.length == 2) {
            oscClient.send(sc_path("/control"), msg[0], msg[1]);
        }
    });
...
function notify_SC_OnBoot() {
    osc_client.send(sc_path("/control"), 'nodejs_boot');
}

function notify_SC_OnClientConnect(socket) {
    var addr = socket.request.connection.remoteAddress.substring(7);
    if (!addr) addr = "127.0.0.1";
    postmsg('connected:    ' + addr);
    osc_client.send(sc_path("/control"), 'client_connect', addr);
}

module.exports.init = init;
module.exports.bindSocket = bindSocket;
module.exports.notify_SC_OnBoot = notify_SC_OnBoot;
module.exports.notify_SC_OnClientConnect = notify_SC_OnClientConnect;
